...
class ConvHull(object):
    ...
    def convexHull(self,cloud):
        ...
        for i in range (2,len(cloud)):
            ...
            while (len(polygonL)>2 and (polygonL[-1]>polygonL[-2]) and not self.isRightTurn((polygonL[-3],polygonL[-2],polygonL[-1]))):
                del polygonL[-2]
        ...
        for i in range (2,len(cloud)):
            ...
            while (len(polygonU)>2 and (polygonU[-1]<polygonU[-2]) and not self.isRightTurn((polygonU[-3],polygonU[-2],polygonU[-1]))):
                del polygonU[-2]
